...
当一条查询到达叶节点时，DBMS 需要从表中实际读取数据。这一节讨论数据访问的基本方式——顺序扫描与索引扫描，以及它们在不同场景下的代价权衡。

### 3.1 查询代价衡量

在深入具体的访问方法之前，先了解 DBMS 评估查询代价的基本模型。对于磁盘数据库，最主要的代价是 **I/O 代价**，通过两个指标衡量：

- **块传输次数**（number of block transfers）：需要从磁盘读取/写入多少个数据块
- **随机 I/O 次数**（number of random I/O accesses / seeks）：需要多少次磁盘寻道

总 I/O 时间可表示为 `b × t_T + S × t_S`，其中 `t_T` 是传输一个块的平均时间，`t_S` 是平均寻道时间（寻道 + 旋转延迟）。对于 SSD，虽然不需要寻道，但发起 I/O 请求到第一个字节返回的延迟仍然存在，同样用 `t_S` 表示。

内存缓冲区的大小会显著影响算法成本——理想情况下所有数据都在内存中，最坏情况下每次只能缓存少量数据块。B+ 树索引通常假设内部节点已在内存缓冲区中，遍历索引只需一次对叶子节点的随机 I/O。

### 3.2 顺序扫描（Sequential Scan / Table Scan）

顺序扫描是最基础的访问方法：从第一个 page 开始，依次读取表中的每个 page，检查其中每个 tuple 是否满足过滤条件。
...
**缺点**：
- 需要扫描全表，即使匹配的 tuple 很少
- 对于选择性高的点查询非常低效

现代 DBMS 对顺序扫描有多种优化手段：

- **预取（Prefetching）**：提前将后续 page 加载到内存，避免 I/O 阻塞
- **缓存池旁路（Buffer Pool Bypass）**：扫描算子将获取的 page 直接放在本地内存而非全局缓冲池，避免"顺序洪水"污染缓存池
- **并行化（Parallelization）**：使用多线程/进程并行扫描不同的数据段
- **晚物化（Late Materialization）**：在列存储中，延迟拼接完整 tuple 直到查询计划的上层，让中间算子只传递最少必要信息（如记录 ID、列偏移）
- **堆聚集（Heap Clustering）**：按聚集索引的顺序物理存储 tuple，使顺序扫描的访问模式更优

### 3.3 索引扫描（Index Scan）

索引扫描利用 B+ 树等索引结构定位到满足条件的 tuple，避免全表扫描。

**聚集索引（Clustered Index）与非聚集索引（Secondary Index）**：

在聚集索引中，数据按索引键的物理顺序存储，满足同一条件的 tuple 分布在相邻的 page 中，读取效率高。在非聚集索引（二级索引）中，索引只存储指向实际 tuple 的指针，匹配的 tuple 可能分散在不同的 page 中，每条记录都需要一次随机 I/O。

**不同条件类型的索引使用策略**：

- **等值条件 + 聚集索引（键属性）**：一次 B+ 树遍历定位到目标，读取一个 tuple，代价为 `(h + 1) × (t_S + t_T)`（`h` 为 B+ 树高度）
- **等值条件 + 聚集索引（非键属性）**：查找第一个匹配 tuple 后顺序扫描连续数据块
- **等值条件 + 二级索引**：每检索一个 tuple 需要一次 I/O，代价为 `(h + n) × (t_S + t_T)`（`n` 为匹配 tuple 数）。当 `n` 较大时代价极高
- **比较条件（`A > v` 等）**：聚集索引下先定位到 `v` 再顺序扫描至末尾；二级索引下很少使用比较条件，除非匹配记录极少

**索引扫描 vs 顺序扫描**：索引扫描不总是更优的选择。如果查询需要访问表中大部分数据（约超过 10–20% 的 tuple），顺序扫描的连续 I/O 会比索引扫描的随机 I/O 更快。PostgreSQL 优化器在估计查询超过约 10% 的表行时会默认选择顺序扫描。

### 3.4 位图索引扫描（Bitmap Index Scan）

当查询涉及多个条件且无法确定索引扫描和顺序扫描哪个更优时，PostgreSQL 使用位图索引扫描作为混合策略：

1. 对每个条件分别使用对应的索引，获取匹配 tuple 的 page ID 集合
2. 构建位图：每一位代表一个 page 是否被匹配
3. 对多个位图取**交集**（合取条件）或**并集**（析取条件）
4. 按 page 物理顺序依次读取标记的 page，在内存中检查具体条件

位图索引扫描既避免了纯索引扫描的大量随机 I/O（因为按物理顺序读取），又避免了纯顺序扫描的全表遍历（因为只读取匹配的 page）。

### 3.5 区域映射（Zone Map）

区域映射是一种**无损数据跳过**（lossless data skipping）技术：

- 为每个 page（或一组 page）预计算其中各列的最小值、最大值等聚合值
- 这些聚合值存储在一个独立的 zone map page 中
- 扫描时先检查 zone map：如果查询条件的值范围与某个 page 的值范围无交集，则直接跳过该 page

例如，查询 `WHERE age > 30`，某个 page 的 zone map 显示该页 `age` 列最大值是 25，则该 page 可被完全跳过。区域映射在云数据库系统中尤为重要，因为减少网络数据传输能显著降低成本。

### 3.6 复杂选择条件

以上讨论主要针对简单的等值或比较条件。对于更复杂的谓词：

**合取条件（`θ₁ AND θ₂ AND ... AND θₙ`）**：

- 从每个简单条件中选出代价最低的索引访问路径
- 用该索引检索匹配的 tuple
- 在内存缓冲区中对检索到的 tuple 验证其余条件

**析取条件（`θ₁ OR θ₂ OR ... OR θₙ`）**：

- 对有索引的每个条件分别进行索引扫描
- 对结果取并集（可使用位图并集避免重复）

**复合索引**：如果存在覆盖多个属性的复合索引（如 `(A, B)` 上的索引），合取条件 `A = 1 AND B = 2` 可直接通过一次索引查找解决。
